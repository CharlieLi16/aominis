...
def ocr_image():
    ...
    Request body:
    {
        ...
        "format": "latex" | "plain"  (default: "latex")
    }
    
    Returns:
    {
        "success": true,
        "text": "extracted problem text in LaTeX",
        "raw": "original GPT response"
    }
    """
    client = get_openai_client()
    if not client:
    ...
    output_format = data.get('format', 'latex')
    ...
    try:
        ...
        if output_format == 'latex':
            format_instruction = """Output the problem using LaTeX notation:
- Use $...$ for inline math
- Use standard LaTeX commands like \\frac{}{}, \\int, \\sum, \\lim, etc.
- Keep explanatory text outside of math delimiters"""
        else:
            format_instruction = "Output in plain text, using ^ for exponents and standard mathematical notation."
        ...
        response = client.chat.completions.create(
            ...
            messages=[{
                ...
                "content": [
                    {
                        "type": "text",
                        "text": f"""Extract the math problem from this image. 
                        
Rules:
1. Extract ONLY the problem/question, not any existing answers or work shown
2. If there are multiple problems, extract all of them separated by newlines
3. Preserve mathematical notation accurately
...
def receive_problem_webhook():
    ...
    try:
        ...
        # Submit to chain synchronously (PythonAnywhere doesn't support background threads well)
        logger.info(f"[WEBHOOK] submit_to_chain={submit_to_chain} (type={type(submit_to_chain).__name__})")
        
        if submit_to_chain:
            logger.info(f"[WEBHOOK] Initializing SDK...")
            sdk = init_sdk_if_needed()
            if not sdk:
                logger.error(f"[WEBHOOK] SDK initialization failed!")
                result['chain_error'] = 'SDK not available'
                webhook_solution_status[order_id]['status'] = 'sdk_failed'
                return jsonify(result)
            logger.info(f"[WEBHOOK] SDK ready: {sdk.address}")
            ...
            try:
                ...
                try:
                    ...
                    # Wait for commit to propagate
                    logger.info(f"[WEBHOOK] Waiting 3s before reveal...")
                    loop.run_until_complete(asyncio.sleep(3))
                    ...
                    if reveal_receipt.success:
                        ...
                        logger.error(f"[WEBHOOK] Reveal may have failed for #{order_id}")
                finally:
                    loop.close()
                    
            except Exception as e:
                logger.error(f"[WEBHOOK] Chain submission error: {e}")
                import traceback
